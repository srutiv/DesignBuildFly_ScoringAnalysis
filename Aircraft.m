classdef Aircraft
    properties
        ...
        run %% struct that gives initial conditions
    ...
    methods
        %% CONSTRUCTOR
        function obj=Aircraft(Wings,Htails,Vtails,Case,Bodies,CG,M,Inert,Name)
            obj.wings=Wings;
            obj.htails=Htails;
            obj.vtails=Vtails;
            obj.bodies=Bodies;
        ...
        %% BUILD CASE FILE
        %function build_case(obj)
        %    string=['.\AVL_FILES\',obj.name,'.run'];
        %    fid=fopen(string,'wt');
        %    fclose(fid);
        %    fid=fopen(string,'w');
        %    case_main(obj.runcase, fid);
        %    fid=fclose(fid);
        %end
        ...
        function obj=run_avl(obj)
            ...
            newrunAVL(filename,basename,obj.run);
%             st_file=['.\AVL_DATA\',obj.name,'_Data.st'];
%             sb_file=['.\AVL_DATA\',obj.name,'_Data.sb'];
%             eig_file=['.\AVL_DATA\',obj.name,'_Data.eig'];
%             obj.st=parseST(st_file);
%             obj.run=parseSB(sb_file);
