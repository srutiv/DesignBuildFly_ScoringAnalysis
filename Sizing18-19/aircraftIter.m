...
%             Clmaxt=CLa*(alpha(clt==cl0t)-alpha0); % 3d wing coefficient of lift (AR>3)
        ...
        function [optimal] = wing_spar(~,M,b,hmax,Lfactor)
            % Finds Optimal Spar Material & Dimensions
            % Less than 2% of Span, Deflection at Wing Tips
            % M = Mass of Plane (kg)
            % b = Span of Plane (m)
            % hmax = Maximum Height of Wing Spar (m)
            % Lfactor = Constant multiplied by Weight of Plane P
            % Ex. Lfactor = 1 (Cruise), Lfactor = cos(Pitch Angle)/cos(Bank Angle)
            
            % Initialize Tracking Variables
            optimal = {'none',inf,inf,inf}; %{Material,Mass(kg),Height(m),Thickness(in)}
            scr = {'none',inf,inf,inf}; %{Material,Mass(kg),Height(m),Thickness(m)}
            
            % Calculate Force of Plane from Mass
            P = M*9.81;
            
            % Iterate Height and Thickness of Spar
            for h = (5e-3):0.001:hmax
                % t = Thickness of Common Sheeting Options(in)
                for t = [0.0625,0.09375,0.125,0.1875,0.25,0.375,0.5]
                    % thick = Thickness of Common Sheeting Options (m)
                    thick = t*25.4*(1e-3);
                    
                    % Calculate Necessary Elastic Modulus
                    E = (12/((h^3)*thick))*((125*Lfactor*P*(b^2))/192);
                    
                    % Find Necessary Material & Calculate Mass of Spar Combination
                    if E <= 7.2e9 % Balsa
                        density = 240;
                        mass = density*thick*h*b;
                        scr = {'balsa',mass,h,thick};
                    elseif E <= 14.39e9 && E > 7.2e9 % Spruce
                        density = 460;
                        mass = density*thick*h*b;
                        scr = {'spruce',mass,h,thick};
                    elseif E <= 20.6e9 && E > 14.39e9 % Oak
                        density = 850;
                        mass = density*thick*h*b;
                        scr = {'oak',mass,h,thick};
                    elseif E <= 71e9 && E > 20.6e9 % Carbon fiber
                        density = 1.68e3;
                        mass = density*thick*h*b;
                        scr = {'carbon fiber',mass,h,thick};
                    end
                    
                    % Find Minimum Weight Combination
                    if cell2mat(scr(2)) < cell2mat(optimal(2))
                        optimal = scr;
                    end
                end
            end
