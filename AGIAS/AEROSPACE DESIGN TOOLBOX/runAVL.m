...
function runavl(filename,basename,aircraft)
%runAVL.m
%
...
%  allegro.avl
%  allegro.mass
beta=aircraft.param.beta;
alpha=aircraft.param.alpha;
use_alpha=aircraft.param.use_alpha;
velocity=aircraft.param.v;
...
%fprintf(fid, '%s\n',   'd1 pm 0'); %Set surface 2 so pitching moment is 0
if use_alpha==1
fprintf(fid, 'a a %.3f\n',alpha);
end
...
%% Execute Run
%Run AVL using 
[~,~] = dos(strcat(avlLocation,' < ',basename,'.run'));
end
